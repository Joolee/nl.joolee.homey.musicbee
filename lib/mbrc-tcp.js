...
function MusicBee(device_data, device_settings) {
	...
	// [Default] null = Update timer (if running)
	// true = Start timer
	// false = Cancel timer
	function stopAfterCurrent(noCancel, ignorePaused) {
		noCancel = typeof noCancel !== 'undefined' ? noCancel : null;
		ignorePaused = typeof ignorePaused !== 'undefined' ? ignorePaused : false;
		
		if(stopTimer !== null && typeof stopTimer == 'object')
		{
		...
		if(noCancel === true)
		{
			...
			else if(!ignorePaused && connected && playerStatus.playerstate.toLowerCase() == 'paused')
			{
				stopTimer = 'paused';
			}
			else // stopped
